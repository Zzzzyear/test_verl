...
# [任务] 你现在实际用的 task 名（来自 egpo_train_config.yaml 的 tasks）
...
EXP_NAME="${TASK}_${MODE}_dapo_${TIMESTAMP}"
...
# --- 5.1 Qwen3 thinking 开关（只在 CMD_ARGS 末尾追加 override） ---
IS_QWEN3=0
...
if [ -n "$ENABLE_THINKING_OVERRIDE" ]; then
  ...
  if [ "$IS_QWEN3" -eq 1 ]; then
    ...
    echo "   🧠 non-Qwen3 model detected; thinking override skipped"
  fi
fi
